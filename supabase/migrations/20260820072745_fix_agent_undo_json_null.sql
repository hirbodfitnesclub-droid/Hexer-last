...
-- JSON null is a scalar, so expanding it with jsonb_array_elements_text fails.
-- Restore arrays only when the captured snapshot actually contains an array.
  ...
  if v_receipt.undo_kind = 'delete_created' then
    ...
    if v_receipt.entity_type = 'task' then
      update public.tasks set
        ...
        checklist = case when jsonb_typeof(v_receipt.before_state->'checklist') = 'array' then v_receipt.before_state->'checklist' else '[]'::jsonb end, updated_at = now()
