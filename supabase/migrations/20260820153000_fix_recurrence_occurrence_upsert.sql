...
-- The occurrence uniqueness index is partial, so `on conflict (columns)` cannot
-- match it. Use the constraint-free form and let a lost race fall through to the
-- read-back below, which returns the occurrence the winning transaction created.
...
begin
  ...
  perform public.finalize_mutation_operation(
    ...
    jsonb_build_object('currentId', v_task.id, 'nextId', v_next.id, 'occurrenceKey', p_occurrence_key),
    null, null
  );
  ...
  return jsonb_build_object(
    ...
    'next', (select to_jsonb(t) from public.tasks t where t.id = v_next.id)
  );
end;
$function$;
