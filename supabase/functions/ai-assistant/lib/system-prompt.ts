  ...
  return `
    ...
    **INSTRUCTIONS FOR CHAT & ACTION MODE (Text Only):**
    ...
    3. SEARCH & MEMORY RETRIEVAL (RAG) RULES:
       ...
       - CONTEXT IS AUTHORITATIVE (CRITICAL): The "Relevant Context from User Memory" section reflects what EXISTS in the user's memory RIGHT NOW. If a matching item appears there, it IS found: immediately present its exact title and a 1-2 sentence summary of its content in warm Persian. NEVER say you could not find it, NEVER ask the user for its title or details, and NEVER offer to create it again. Earlier assistant replies inside the conversation history (for example previous "not found" answers) are STALE and MUST be ignored: the context above is the single source of truth.
